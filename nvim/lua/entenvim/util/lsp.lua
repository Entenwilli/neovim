...
function M.rename_file()
	...
	local old = assert(EntenVim.root.realpath(vim.api.nvim_buf_get_name(buf)))
	local root = assert(EntenVim.root.realpath(EntenVim.root.get({ normalize = true })))
...
function M.formatter(opts)
	...
	local ret = {
		...
		priority = 1,
		format = function(buf)
			M.format(EntenVim.merge({}, filter, { bufnr = buf }))
		end,
		sources = function(buf)
			local clients = M.get_clients(EntenVim.merge({}, filter, { bufnr = buf }))
			---@param client vim.lsp.Client
	...
	return EntenVim.merge(ret, opts) --[[@as LazyFormatter]]
end
...
function M.format(opts)
	opts = vim.tbl_deep_extend(
		...
		{},
		opts or {},
		EntenVim.opts("nvim-lspconfig").format or {},
		EntenVim.opts("conform.nvim").format or {}
	)
	...
	M.on_supports_method("textDocument/documentHighlight", function(_, buf)
		vim.api.nvim_create_autocmd({ "CursorHold", "CursorHoldI", "CursorMoved", "CursorMovedI" }, {
			...
			callback = function(ev)
				if not require("entenvim.user.keymaps").has(buf, "documentHighlight") then
					return false
				end
				...
				if not ({ M.words.get() })[2] then
					if ev.event:find("CursorMoved") then
						vim.lsp.buf.clear_references()
					elseif not EntenVim.cmp.visible() then
